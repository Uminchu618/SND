...
class ExperimentNEnvPPO:
    ...
    def test(self, agent):
        ...
        for i in range(1):
            ...
            state0 = (
                torch.tensor(self._env.reset(), dtype=torch.float32)
                .unsqueeze(0)
                .to(config.device)
            )
            done = False
            ...
            while not done:
                ...
                next_state, reward, done, info = self._env.step(
                    agent.convert_action(action0.cpu())[0]
                )
                state0 = (
    ...
    def run_baseline(self, agent, trial):
        ...
        while step_counter.running():
            ...
            analytic.update(re=reward, ext_value=value[:, 0].unsqueeze(-1).cpu())
            ...
            for i, index in enumerate(env_indices):
                ...
                print(
                    "Run {0:d} step {1:d}/{2:d} training [ext. reward {3:f} steps {4:d}  mean reward {5:f} score {6:f})]".format(
                        trial,
                        step_counter.steps,
                        step_counter.limit,
                        stats["re"].sum[i],
        ...
        agent.save(
            "./models/{0:s}_{1}_{2:d}".format(self._env_name, config.model, trial)
        )
        ...
        numpy.save(
            "ppo_{0}_{1}_{2:d}".format(config.name, config.model, trial), save_data
        )
        analytic.clear()
    ...
    def run_rnd_model(self, agent, trial):
        ...
        while step_counter.running():
            ...
            for i, index in enumerate(env_indices):
                ...
                print(
                    "Run {0:d} step {1:d}/{2:d} training [ext. reward {3:f} int. reward (max={4:f} mean={5:f} std={6:f}) steps {7:d}  mean reward {8:f} score {9:f})]".format(
                        trial,
                        step_counter.steps,
                        step_counter.limit,
                        stats["re"].sum[i],
    ...
    def run_snd_model(self, agent, trial):
        ...
        while step_counter.running():
            ...
            done_tensor = torch.tensor(done, dtype=torch.float32).view(
                -1, 1
            )  # Ensure shape is (128, 1)
            prev_level_complete_tensor = torch.tensor(
                info["prev_level_complete"], dtype=torch.float32
            ).view(-1, 1)
            local_step_count = analytic.get_counter()
            # 死亡した場合の報酬を変更する
            # 死亡の判定条件　クリアしていない、ゲームが終了が1、local_step_countがタイムアウトまでいたっていない
            death = (
                (prev_level_complete_tensor == 0)
                & (done_tensor == 1)
            ...
            for i, index in enumerate(env_indices):
                ...
                print(
                    "Run {0:d} step {1:d}/{2:d} training [ext. reward {3:f} int. reward (max={4:f} mean={5:f} std={6:f}) steps {7:d}  mean reward {8:f} score {9:f} feature space (max={10:f} mean={11:f} std={12:f})] complete={13} seed={14} mask={15}".format(
                        ...
                        info["prev_level_complete"][index],
                        info["prev_level_seed"][index],
                        death[index].item(),
                    )
                )
                print(time_estimator)
    ...
    def run_sp_model(self, agent, trial):
        ...
        while step_counter.running():
            ...
            ext_reward = torch.tensor(reward, dtype=torch.float32)
            int_reward = (
                agent.motivation.reward(state0, action0, self.process_state(next_state))
                .cpu()
                .clip(0.0, 1.0)
            )
